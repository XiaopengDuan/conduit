  ...
  <div style="height:100%" v-loading="loading">
    ...
    <div class="list-ys-box list-yhmes-box chart-wrap-box">
      ...
      <div class="up-box">
        ...
        <div class="chart-wrap">
          <!-- <img width="100%" height="100%" src="../../assets/img/blueChart.png"/>-->
          <div id="container2" class="echarts-box" style="width:100%;height:2.5rem"></div>
        </div>
      </div>
    </div>
    <!--右侧page-->
    <rightNav :menuIndex="5"></rightNav>
    <!--右侧page//-->
    <!--右侧切换按钮-->
    <router-link
      name="切换"
      class="btn-switch"
      :to="{path:`/userCenter/detail/${this.$route.params.userNo}/p5_2`}"
    ></router-link>
    <!--右侧切换按钮//-->
  </div>
</template>
...
import { getUserWaterQopBar, getUserMoneyQopBar } from "@/api/usercenter";
...
export default {
  ...
  methods: {
      ...
      getUserWaterQopBar(postData)
        .then(res => {
          if (res.code === 200) {
            ...
            this.drawWaterLine(container1, options);
          }
          that.loading = false;
        })
        .catch(res => {
    ...
    loadCurveMoney: function() {
      ...
      postData.startMonth = this.startMonth1;
      postData.endMonth = this.endMonth1;
      getUserMoneyQopBar(postData).then(res => {
        if (res.code === 200) {
          let data = res.data;
          let options = {
            months: data.months,
          ...
          this.drawWaterLine2(container2, options);
        }
      });
    },
    ...
    drawWaterLine2(container, options) {
      echarts.init(container).setOption({
        grid: {
          top:10,
          left: 80,
