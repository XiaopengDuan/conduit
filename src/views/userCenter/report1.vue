  ...
  <div style="height:100%" :class="{'user-list':true,'collapse':collapse,'uncollapse':!collapse}">
    ...
    <div class="bottom-table-box bottom-table-box-khbb">
      ...
      <div class="col-8 table-wrap">
        ...
        <table class="table-padding table table-striped " v-loading="loading">
          <thead>
            <tr>
              <th width="100rem">用户名</th>
...
import {
  ...
  getUserNormalPayReportInfoList
} from "@/api/usercenter";
export default {
  name: "Home",
  components: {
  ...
  data() {
    return {
      ...
      tableTiltes: [
        ...
        { name: "用户类型", code: "userTypeName" },
        { name: "联系电话", code: "contactMobile" },
        { name: "累计用水量", code: "totalPower" },
        { name: "上月用水量", code: "writeMonth" },
        { name: "同比增长", code: "writeYty" },
        { name: "环比增长", code: "writeQoq" },
        { name: "累计水费", code: "totalMoney" },
        { name: "上月水费", code: "moneyMonth" },
        { name: "水费同比增长", code: "moneyYty" },
        { name: "水费环比增长", code: "moneyQoq" }
      ],
      show: false,
      pageSize: 10,
      pageNow: 1,
  ...
  methods: {
    ...
    setPagerParams: function(pageNow, pagerId) {
    ...
    loadData: function() {
      ...
      getUserNormalPayReportInfoList(postData)
        .then(response => {
