...
<script>
  ...
  import { getContentPriceArrearCountList} from '@/api/charge'
  ...
  export default {
    name: "arrearResidentialquarters", // 2,4,6,8,10,14,16,19,20
    data(){
      return {
    ...
    mounted(){
      //初始化分页查询
      initPage(this.temp, getContentPriceArrearCountList, 'countLists')
    ...
    methods: {
      ...
      link_arrarDiagram(value){
        this.$router.push({
          ...
          params: {userNo: '**', userName: '**',communityCode: 'null',contentPriceCode: value.code,elecTypeCode: 'null',type:'arrearResidentialquarters'}
        });
        ...
        this.$router.push({
          name: "arrearUserInfo",
          params: {seachItems: JSON.stringify({contentPriceCode: value.code, month: this.temp.countLists[0].months[this.startNumber+index]})}
        });
      },
