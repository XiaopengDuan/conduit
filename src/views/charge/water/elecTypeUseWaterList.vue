  ...
  <div class="right right2">
    <div class="right-content">
      ...
      <div class="tablecont-box" v-if="temp.countLists.length===0 || Object.keys(temp.countLists[0].countsAndPowers).length>0">
        <div class="sharetitle">不同用水性质用水量列表
        ...
        <!-- <div class="scroll-box"> -->
         <table  class="table table-jf table-fx" width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr v-for="(value, index) in temp.countLists" :key="index">
              <td align="center" class="text-success" width="1rem">
                <el-tooltip :content="value.name" effect="light" placement="top-start">
                  <a>{{value.name | tooltip-filter}}</a>
                </el-tooltip>
              </td>
              <td align="center" width="1rem">{{value.totalPowers}}</td>
...
<script>
  ...
  import { getElecTypeUseWaterList} from '@/api/chargeWater'
  ...
  export default {
    ...
    created() {
      initPage(this.temp, getElecTypeUseWaterList, 'countLists')
      this.temp.page.pageSize = 10;
      // this.temp.seachItems.year = '2019';
    ...
    methods: {
      ...
      link_CurveJump(value){
        ...
        this.$router.push({
          ...
          params: {userNo: '**', userName: '**', seachItems: JSON.stringify({elecTypeCode: value.code}), type:'analysisUseWater'}
        });
      },
      link_userRanking(value,e){
        ...
        this.$router.push({
          ...
          params: {seachItems: JSON.stringify({elecTypeCode: value.code})}
        });
      },
