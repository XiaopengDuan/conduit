            ...
            <tbody>
              <tr v-for="(value, index) in temp.queryData" key="code">
                ...
                <template v-for="(v,i) in value.cellList.slice(startNumber,startNumber + 5)">
                  <td class="detailLink" @click="link_detail_table(value, v.element.totalMoney)">￥{{v.element.totalMoney}}</td>
                  <td
                    class="detailLink"
                    style="color: red"
                    @click="link_detail_table(value,v.element.chargeMoney)"
...
import { get51IncomeByContentPriceCode } from "@/api/charge";
...
export default {
  ...
  mounted() {
    // 初始化分页
    initPage(this.temp, get51IncomeByContentPriceCode, "queryData");
    this.temp.page.pageSize = 8;
    // 设置查询条件
    this.temp.seachItems.startMonth = "201801";
    this.temp.seachItems.endMonth = "201905";
    // 查询第一页
  ...
  methods: {
    ...
    link_detail_page(value) {
      ...
      this.$router.push({
        ...
        params: { typeCode: value.typeCode, typeName: value.typeName, type:'WaterType' }
      });
    },
