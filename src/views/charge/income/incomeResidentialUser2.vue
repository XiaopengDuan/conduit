      ...
      <div class="tablecont-box" v-if="temp.queryData.length===0 || temp.queryData[0].cellList.length>0">
          ...
          <div class="input-group-btn">
            <el-tooltip class="item" content="按回车默认查询" placement="top-start" effect="light">
              <el-input
                ...
                @keyup.enter.native="temp.searchCurrent(1)"
                placeholder="请输入用户名"
              ></el-input>
            </el-tooltip>
          </div>
        ...
        <!-- <div class="scroll-box"> -->
          ...
          >
            <thead>
              <tr>
                <th rowspan="2" scope="col">小区名称</th>
              ...
              <tr v-for="(value, index) in temp.queryData" key="code">
                <td align="center" class="text-success" width="2rem">
                  <el-tooltip :content="value.typeName" effect="light" placement="top-start">
                    <a style="cursor: pointer">{{value.typeName | tooltip-filter(2)}}</a>
                  </el-tooltip>
                </td>
                <td align="center" width="1rem">{{value.userNo}}</td>
                <td align="center" width="1rem">{{value.userName}}</td>
...
import { get32UserPayByCommunity } from "@/api/charge";
...
export default {
  ...
  created() {
    // 加载分页方法
    initPage(this.temp, get32UserPayByCommunity, "queryData");
    ...
    this.temp.seachItems.communityCode = this.$route.params.typeCode+''
    this.titleName = this.$route.params.typeName
    // 查询第一页
  ...
  methods: {
    ...
    tempsearchN() {
      ...
      this.communityOption.forEach(res=>{
        if(res.communityCode === this.temp.seachItems.communityCode) {
          this.temp.seachItems.typeName = res.communityName
          this.titleName = res.communityName
          this.temp.seachItems.typeCode = res.communityCode
          this.temp.searchN()
        }
      })
    },
