  ...
  <div class="right right3">
    <div class="right-content">
      ...
      <div class="tablecont-box" v-if="temp.queryData.length===0 || temp.queryData[0].cellList.length>0">
        ...
        <div class="sharetitle">{{$route.params.typeName}}收益排行
          ...
          <div class="input-group-btn">
          <el-input v-model="temp.seachItems.username" @keyup.enter.native="tempsearchN" placeholder="请输入用户名"></el-input>
         </div>
          <div class="input-group-btn">
        <el-input v-model="temp.seachItems.userno" @keyup.enter.native="tempsearchN"  placeholder="请输入户号"></el-input>
         </div>
        </div>
        ...
        <!-- <div class="scroll-box"> -->
          <table  class="table table-jf table-fx" width="100%" border="0" cellspacing="0" cellpadding="0">
            <thead>
            <tr>
              <th rowspan="2" scope="col">水价类型</th>
              ...
              <th colspan="2" scope="col" v-for="(value,index) in temp.queryData[0].cellList.slice(startNumber,startNumber + 4)" :key="index">
                <span class="d4" v-if="index===0 && startNumber!==0 " @click="updateStartNumber(-4)"></span>
                <span class="d3" v-else-if="index===3 && startNumber + 4 < temp.queryData[0].cellList.length" @click="updateStartNumber(4)"></span>
                {{value.month}}
              </th>
            ...
            <tr v-for="(value, index) in temp.queryData" :key="index">
              ...
              <td align="center" class="chart-td" @click="linkGraph(value)"><div :id="'echarts'+ index " style="width: 336px;height: 34px">{{darwEcharts(index, value.graph)}}</div></td>
...
<script>
  ...
  import { get42UserPayByUserType} from '@/api/charge'
  ...
  export default {
    ...
    created() {
      ...
      // 加载分页方法
      initPage(this.temp, get42UserPayByUserType, 'queryData')
