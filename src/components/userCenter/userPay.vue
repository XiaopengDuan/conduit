<template>
  ...
  <div class="model" style="display: block" v-show="isShowPanel">
    <div class="model-box">
      ...
      <div class="content">
        <div class="line-7">
          <div class="title">水费记录</div>
          <div class="content-mes">
            <div class="input-group">
              <div class="col-12">
                <label class="label-text">月均水费</label>
                <input type="text" class="form-control" :value="monFee" disabled>
              </div>
              <div class="col-12">
                <label class="label-text">全年水费</label>
                <input type="text" class="form-control" :value="yearFee" disabled>
              </div>
              <div class="col-12">
                <label class="label-text">最少月份水费</label>
                <input type="text" class="form-control" :value="minFee" disabled>
              </div>
              <div class="col-12">
                <label class="label-text">最大月份水费</label>
                <input type="text" class="form-control" :value="maxFee" disabled>
              </div>
            </div>
          </div>
        </div>
        <div class="line-5">
          <div class="title">上月同小区水费对比</div>
          <div class="content-mes">
...
import { getMoneyWindowInfoPage1 } from "@/api/usercenter";
export default {
  props: {
    userNo: {
      default: ""
  ...
  mounted() {
    ...
    getMoneyWindowInfoPage1(postData)
      .then(response => {
        if (response.code == 200) {
          that.monFee = response.data.monenys.月均水费;
          that.yearFee = response.data.monenys.全年水费;
          that.minFee = response.data.monenys.最少月份水费;
          that.maxFee = response.data.monenys.最高月份水费;
  ...
  methods: {
    ...
    initChart: function() {
      ...
      var myChart = echarts.init(document.getElementById("pay_chart"));
      // 绘制图表
      myChart.setOption({
        title: {
          text: "单位：用户量",
